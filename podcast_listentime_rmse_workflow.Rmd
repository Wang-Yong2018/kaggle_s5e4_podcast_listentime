...
col_name <-'dt'
train|>
  select(publication_day,publication_time,listening_time_minutes)|>
...
od_train <- train |>get_outliers()|>select(score_pctl)  |>bind_cols(train)
od_test <- test|>get_outliers()  |>select(score_pctl) |>bind_cols(test)
...
diag_mod_fit <- function(wf){
  ...
  p1 <- 
    aug_df |>
    select(id, .pred,actual_y=listening_time_minutes)|>
  ...
  p2 <-
    aug_df |>
    select(id, .resid,actual_y=listening_time_minutes)|>
