...
get_rcp <- function(df) {
  rcp <-
    ...
    #step_sqrt(listening_time_minutes, skip=TRUE)|>
    step_mutate(
      
      listening_time_minutes=rescale(listening_time_minutes,to=c(0,1),from=c(0,120)),
      listening_time_minutes = (listening_time_minutes*(n-1) + 0.5)/n, # should be larget than 0 and less than 1
      skip=TRUE)|>
    #step_log(listening_time_minutes, offset=1, skip=TRUE)|>
    #step_mutate(score_pctl = get_outliers(.x))|>
  ...
  #  step_mutate(#episode_id =readr::parse_number(episode_title))|>
...
mm_fit <- 
  ...
  mutate(.pred=list(get_predict(model, data, target_type = 'beta'))) |>
  rowwise()|>mutate(rmse_score = yardstick::rmse_vec(truth=true, estimate=.pred))

tmp_true <- mm_fit |> pluck('true',1)
tmp_pred <- mm_fit |> pluck('.pred',1)
tmp_target <- tibble("true"=tmp_true, "pred"=tmp_pred)
tmp_target |> 
  pivot_longer(cols=everything())|>
  ggplot(aes(x=value,color=name))+geom_density()

tmp_mod <- mm_fit$model[[1]] |>extract_fit_engine()
#tmp_mod |> tidy()
#tmp_mod|>glance()
